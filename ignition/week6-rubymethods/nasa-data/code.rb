...
# Your code here
def location(array)
  array.each do |big_company_hash|
    big_company_hash.each do |company_name,hash|
      if hash["location_1_state"].nil?
        hash["location_1_state"] = ""
      end
      puts "#{company_name}: #{hash.fetch("location_1_city")}, #{hash.fetch("location_1_state")}"
    end
  end
end
...
def id_number(array)
  array.each do |hash|
    hash.each do |company_name,hashmap|
      puts "#{company_name}: #{hashmap.fetch("computed_region_cbhk_fwbd")}"
    end
  end
end
...
def position(array)
  array.each do |hash|
    hash.each do |company_name,hashmap|
      puts "#{company_name}: #{hashmap.fetch("position")}"
    end
  end
end
...
def type(array)
  array.each do |hash|
    hash.each do |company_name,hashmap|
      hashmap.each do |a,b|
        if b.is_a?(Hash)
          puts "#{company_name}: #{b.fetch("type")}"
        end
      end
    end
  end
...
def number(array)
  ...
  array.each do |a|
    a.each do |b,c|
      c.each do |d,e|
        if d == "computed_region_nnqa_25f4"
          if e > high
            high = e
            high_company = b
          end
