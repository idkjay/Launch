...
let whoSaidThis = (quote) => {
  for (i=0;i<quotes.length;i++) {
    if (quotes[i].includes(quote))
    return crewMembers[i]
  }
}
whoSaidThis(washQuote)
whoSaidThis(jayneQuote)


let longestQuote = (name) => {
  let i = crewMembers.indexOf(name)
  quotes[i].sort(function(a,b) {
...
let averageQuoteLength = (name) => {
  let i = crewMembers.indexOf(name)
  let totalQuoteLength = quotes[i].reduce(function(sum, quote){
    return sum + quote.length;
  },0);
  let avgQuoteL = (totalQuoteLength/quotes[i].length)
  return Math.round(avgQuoteL)
}
averageQuoteLength("Zoe")
...
console.log(`Mal has ${numberOfQuotes("Mal")} quotes.`) //5
